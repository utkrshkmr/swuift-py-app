...
_TMPR_VALUES: tuple[float, ...] = (28.62, 178.171, 192.151, 306.514, 484.043, 677.377, 680.536, 682.559, 684.021, 685.132, 686.011, 686.728, 687.327, 687.837, 688.278, 688.665, 689.008, 689.314, 689.59, 689.84, 690.068, 690.278, 690.471, 690.649, 690.815, 690.969, 691.113, 691.248, 691.374, 691.493, 691.604, 691.709, 691.808, 691.902, 691.99, 692.074, 678.551)
...
def build_config(defaults: Optional[dict], *, grid_size: Optional[int]=None, t_start: Optional[datetime]=None, t_end: Optional[datetime]=None, maxstep: Optional[int]=None, hardening_rad: Optional[float]=None, hardening_spo: Optional[float]=None, rad_energy_ig: Optional[float]=None, rad_rf: Optional[float]=None, fb_wind_coef: Optional[float]=None, fb_wind_sd: Optional[float]=None, fb_wind_sd_transverse: Optional[float]=None, seed_hardening: Optional[int]=None, seed_spread: Optional[int]=None) -> SWUIFTConfig:
    ...
    rad_energy_ig_val = float(rad_energy_ig) if rad_energy_ig is not None else _scalar_from_defaults('rad_energy_ig', RAD_ENERGY_IG_DEFAULT)
    rad_rf_val = float(rad_rf) if rad_rf is not None else _scalar_from_defaults('rad_rf', RAD_RF_DEFAULT)
    fb_mass = _scalar_from_defaults('fb_mass', FB_MASS)
    fb_wind_coef_val = float(fb_wind_coef) if fb_wind_coef is not None else _scalar_from_defaults('fb_wind_coef', FB_WIND_COEF_DEFAULT)
    fb_wind_sd_val = float(fb_wind_sd) if fb_wind_sd is not None else _scalar_from_defaults('fb_wind_sd', FB_WIND_SD_DEFAULT)
    fb_wind_sd_transverse_val = float(fb_wind_sd_transverse) if fb_wind_sd_transverse is not None else _scalar_from_defaults('fb_wind_sd_transverse', FB_WIND_SD_TRANSVERSE_DEFAULT)
    ...
    hardening_level_rad = float(hardening_rad) if hardening_rad is not None else _scalar_from_defaults('hardening_level_rad', HARDENING_RAD_DEFAULT)
    hardening_level_spo = float(hardening_spo) if hardening_spo is not None else _scalar_from_defaults('hardening_level_spo', HARDENING_SPO_DEFAULT)
    ...
    return SWUIFTConfig(grid_size=grid_size if grid_size is not None else GRID_SIZE, t_start=t_start, t_end=t_end, t_step_min=t_step_min, maxstep=maxstep, aes=aes, ee=ee, er=er, sconst=sconst, rad_energy_ig=rad_energy_ig_val, rad_rf=rad_rf_val, fb_mass=fb_mass, fb_wind_coef=fb_wind_coef_val, fb_wind_sd=fb_wind_sd_val, fb_wind_sd_transverse=fb_wind_sd_transverse_val, fb_dist_mu=fb_dist_mu, fb_dist_sd=fb_dist_sd, veg_included=veg_included, tmpr=tmpr_vec, hardening_level_rad=hardening_level_rad, hardening_level_spo=hardening_level_spo, seed_hardening=seed_h, seed_spread=seed_s)
