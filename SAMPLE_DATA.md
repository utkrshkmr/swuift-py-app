...
repository — it is published as a GitHub Release and downloaded into
`eaton_sample_data/`.
...
`eaton_sample_data/`.  Point Tab 1 of the SWUIFT GUI at that folder.
...
| ------------------------------- | -----------: | ------------------------------------------------------------------- |
| `eaton_sample_data_small.tar.gz`|    ~75 MB    | 9 small input rasters (all `.mat` files except `wind.mat`)          |
...
```bash
mkdir -p eaton_sample_data && cd eaton_sample_data

BASE=https://github.com/utkrshkmr/swuift-py-app/releases/download/sample-data-v1
for f in SHA256SUMS eaton_sample_data_small.tar.gz wind.mat.part.aa wind.mat.part.ab \
         wind.mat.part.ac wind.mat.part.ad; do
    curl -LO "$BASE/$f"
done

shasum -a 256 -c SHA256SUMS               # expect every line to print "OK"
tar -xzf eaton_sample_data_small.tar.gz   # 9 small .mat files
cat wind.mat.part.* > wind.mat            # reassemble
rm wind.mat.part.* eaton_sample_data_small.tar.gz
```
...
- the full, uncompressed `eaton_sample_data/` present locally, including
  `wind.mat`;
...
1. `tar -czf` the small `.mat` files into `eaton_sample_data_small.tar.gz`.
